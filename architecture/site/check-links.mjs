...
if (missing.length) {
  ...
  console.log(
    `::warning::Architecture model has ${missing.length} dead source link(s) — the model may be drifting from the code (these links 404 for viewers; fix in architecture/*.c4):`,
  );
  ...
  console.log(
    `✓ All architecture model source links resolve (${files.length} .c4 file(s) scanned).`,
  );
}
process.exit(0);
