 ...
 * `state.fangfishFieldWatchlist` carries the per-card-id state across
 * the two end-phases.
 *
...
import { applyFangfishFragility } from './slices/zones';
...
describe('applyFangfishFragility — empty / no-fangfish path (a)', () => {
  it('returns input reference unchanged when field has no fangfish and watchlist is empty', () => {
    const s = makeState({ field: [baseReefblade] });
    expect(applyFangfishFragility(s)).toBe(s);
  });
  ...
  it('clears a stale watchlist when no fangfish is in the field anymore', () => {
    ...
    const s = makeState({
      field: [baseReefblade],
      fangfishFieldWatchlist: ['fangfish'],
    });
    const next = applyFangfishFragility(s);
    expect(next).not.toBe(s);
    expect(next.fangfishFieldWatchlist).toEqual([]);
  });
});

describe('applyFangfishFragility — first scan (b)', () => {
  it('populates the watchlist with the fangfish id but does not discard', () => {
    const s = makeState({ field: [baseFangfish, baseReefblade] });
    const next = applyFangfishFragility(s);
    expect(next.field).toHaveLength(2); // both still in field
    expect(next.fangfishFieldWatchlist).toEqual(['fangfish']);
    expect(next.defeated).toHaveLength(0);
  });
});

describe('applyFangfishFragility — second scan, fragility fires (c)', () => {
  it('discards a fangfish that survived a full turn-end into state.defeated', () => {
    const s = makeState({
      field: [baseFangfish, baseReefblade],
      fangfishFieldWatchlist: ['fangfish'],
    });
    const next = applyFangfishFragility(s);
...
describe('applyFangfishFragility — re-engagement path (d)', () => {
  it('a fangfish that left the field between scans is not auto-banished', () => {
    ...
    const s = makeState({
      field: [baseReefblade],
      defeated: [baseFangfish], // already in defeated from the fight
      fangfishFieldWatchlist: ['fangfish'],
    });
    const next = applyFangfishFragility(s);
    expect(next.field).toEqual([baseReefblade]);
...
describe('applyFangfishFragility — multi-fangfish independence (e)', () => {
  it('a fresh fangfish gets its own grace window when an older one fires', () => {
    ...
    const oldFangfish: Card = { ...baseFangfish, id: 'fangfish' };
    const newFangfish: Card = { ...baseFangfish, id: 'fangfish-2' };
    const s = makeState({
      field: [oldFangfish, newFangfish, baseReefblade],
      fangfishFieldWatchlist: ['fangfish'],
    });
    const next = applyFangfishFragility(s);
    ...
    expect(next.fangfishFieldWatchlist).toEqual(['fangfish-2']);
  });

  it('two coexisting fangfish that both survived BOTH fire on the same scan', () => {
    const a: Card = { ...baseFangfish, id: 'fangfish' };
    const b: Card = { ...baseFangfish, id: 'fangfish-2' };
    const s = makeState({
      field: [a, b, baseReefblade],
      fangfishFieldWatchlist: ['fangfish', 'fangfish-2'],
    });
    const next = applyFangfishFragility(s);
    expect(next.field).toEqual([baseReefblade]);
...
describe('applyFangfishFragility — suffix-mint duplicates resolve via baseId (f)', () => {
  it('a suffix-minted "fangfish-3" still triggers the fragility window', () => {
    const minted: Card = { ...baseFangfish, id: 'fangfish-3' };
    // First scan — populates.
    let s = makeState({ field: [minted] });
    s = applyFangfishFragility(s);
    expect(s.fangfishFieldWatchlist).toEqual(['fangfish-3']);
...
describe('applyFangfishFragility — non-Maren robustness', () => {
  it('non-Maren zones are still identity when no fangfish is in the field (sanity)', () => {
    const s = makeState({ currentZone: 'sylvani', field: [baseReefblade] });
    expect(applyFangfishFragility(s)).toBe(s);
  });
