 ...
 *     (Helmaroc King → Voltwyrm → Kalle Demos → Sandscourge → Idolarch),
...
const EXPECTED_ORDER = [
  { spec: COLOSSEUM_HELMAROC_KING_T3, sourceCardId: 'skrall-king' },
  { spec: COLOSSEUM_VOLTWYRM_T3, sourceCardId: 'voltwyrm' },
  { spec: COLOSSEUM_KALLE_DEMOS_T3, sourceCardId: 'vinemaw' },
...
describe('colosseum tier-3 spec — roster shape (embertide-wacl + bngt)', () => {
  ...
  it('Helmaroc King is Layered archetype with mask + head layers (aggregate hp = sum)', () => {
    expect(COLOSSEUM_HELMAROC_KING_T3.archetype).toBe('layered');
    expect(tagKinds(COLOSSEUM_HELMAROC_KING_T3)).toEqual(['layered']);

    const layeredTag = COLOSSEUM_HELMAROC_KING_T3.stateTags?.[0];
    ...
    expect(layerSum).toBe(COLOSSEUM_HELMAROC_KING_T3.hpMax);
  });
  ...
  it('Kalle Demos is Item-Check archetype reusing Cinderwyrm T2 bomb-tag', () => {
    expect(COLOSSEUM_KALLE_DEMOS_T3.archetype).toBe('item-check');
    expect(tagKinds(COLOSSEUM_KALLE_DEMOS_T3)).toEqual(['guarded']);

    const guardedTag = COLOSSEUM_KALLE_DEMOS_T3.stateTags?.[0];
    if (guardedTag?.kind !== 'guarded') throw new Error('expected guarded tag');
  ...
  it('Idolarch is Swarm archetype with 6 sword-arm minions (parallel/independent of torso hp)', () => {
    ...
    // parallel/independent (mirror Dead Hand T2 convention).
